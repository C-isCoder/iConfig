...
function activate(ctx) {
    ...
    if (goInstallTools_2.checkLanguageServer()) {
        const c = new vscode_languageclient_1.LanguageClient('langserver-go', {
            command: util_1.getBinPath('langserver-go'),
            args: [
                '-mode=stdio'
            ],
        }, {
    ...
    ctx.subscriptions.push(vscode.workspace.onDidChangeConfiguration(() => {
        ...
        if (useLangServer !== updatedGoConfig['useLanguageServer'] && (!updatedGoConfig['useLanguageServer'] || goInstallTools_2.checkLanguageServer())) {
